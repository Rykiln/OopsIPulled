...
module.exports = {
  ...
  execute(msgObject, args, client) {
    ...
    const serverEmoji = serverEmojisTotal.filter((e) => e.animated === false).size;
    const serverEmojiAnimated = serverEmojisTotal.filter((e) => e.animated === true).size;
    const serverChannels = msgObject.guild.channels.cache;
    const serverCategories = serverChannels.filter((c) => c.type === 'category');
    let serverCategoriesNearFull = 0;
    const serverCategoriesNearFullName = [];
    const serverCategoriesNearFullSize = [];
